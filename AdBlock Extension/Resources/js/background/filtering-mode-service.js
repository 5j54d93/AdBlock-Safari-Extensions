// AdBlock filtering-mode service. This module owns default and per-site
// protection levels while preserving existing storage data across upgrades.
...
import { applyFilteringModesToDNR } from './ruleset-service.js';
import { hasBroadHostPermissions } from '../shared/ext-utils.js';
...
async function commitFilteringModeDetails(details, { notify = false } = {}) {
    await applyFilteringModesToDNR(details);

    const data = serializeModeDetails(details);
...
export async function readFilteringModeDetails(bypassCache = false) {
    ...
    await applyFilteringModesToDNR(userModes);
...
async function writeFilteringModeDetails(afterDetails) {
    return commitFilteringModeDetails(afterDetails, { notify: true });
}
...
export async function setFilteringModeDetails(details) {
    await commitFilteringModeDetails(details);
}
...
export async function syncWithBrowserPermissions() {
    ...
    const broadHostPermissionsToggled =
        hasBroadHostPermissions !== runtimeSettings.hasBroadHostPermissions;
    let modified = false;
    ...
    if ( broadHostPermissionsToggled ) {
        runtimeSettings.hasBroadHostPermissions = hasBroadHostPermissions;
        saveRuntimeSettings();
    }
